<%@ page title="Task | Task Details | Tasks" language="C#" masterpagefile="~/AdminPage.master" autoeventwireup="true" inherits="TaskDetails, App_Web_tjb0csgq" %>
                                        ...
                                        <asp:UpdatePanel ID="udpvwTask" runat="server">
                                            <ContentTemplate>
                                                <asp:GridView ID="grdvwTask" runat="server" CssClass="mGrid" AutoGenerateColumns=" False"
                                                    ...
                                                    OnRowCommand=" grdvwTask_RowCommand" ShowFooter="True" PageSize="300" AllowPaging="True"
                                                    AllowSorting="true">
                                                    <Columns>
                                                        ...
                                                        <asp:BoundField DataField="TaskName" HeaderText="Task Name" ItemStyle-Width="200px" ItemStyle-HorizontalAlign="Left" />
                                                        ...
                                                        <asp:BoundField DataField="TaskStatus" HeaderText="Task Status" ItemStyle-Width="80px" ItemStyle-HorizontalAlign="Left" />
                                                ...
                                                <asp:SqlDataSource ID="sDsvwTask" runat="server" ConnectionString="<%$ ConnectionStrings:ConnStr %>"
                                                    SelectCommand="SELECT ROW_NUMBER() OVER(ORDER BY TaskID) AS Row , [TaskID] As Code , CASE  WHEN len(TaskName) > 30  THEN SUBSTRING(TaskName,0,30) + '...' ELSE [TaskName] END as [TaskName] ,EserialNo, [CDName] ,convert(varchar, [TaskDate], 103) as  TaskDate, [TaskStatus]  From vwTask  where TaskStatus <> 'Close' Order by Code desc"
